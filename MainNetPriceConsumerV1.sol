...
contract MainNetPriceConsumerV1 {
    ...
    AggregatorV3Interface internal FilEthPriceFeed;
    ...
    constructor() {
        ...
        FilEthPriceFeed = AggregatorV3Interface(0x0606Be69451B1C9861Ac6b3626b99093b713E801);
        ...
        XrpUsdPriceFeed = AggregatorV3Interface(0xCed2660c6Dd1Ffd856A5A82C67f3482d88C50b12);
    ...
    function getLatestAaveEth() public view returns (uint, uint) {
        ( /*uint80 roundID*/, int price, /*uint startedAt*/, uint timeStamp, /*uint80 answeredInRound*/) = AaveEthPriceFeed.latestRoundData();
        uint uintPrice;
        if (price < 0) {uintPrice = uint(-price);}
        else {uintPrice = uint(price);}
        return (uintPrice, timeStamp);
    ...
    function getLatestFilEth() public view returns (uint, uint) {
        ( /*uint80 roundID*/, int price, /*uint startedAt*/, uint timeStamp, /*uint80 answeredInRound*/) = FilEthPriceFeed.latestRoundData();
        uint uintPrice;
        if (price < 0) {uintPrice = uint(-price);}
        else {uintPrice = uint(price);}
        return (uintPrice, timeStamp);
